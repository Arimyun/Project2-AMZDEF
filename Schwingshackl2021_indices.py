...
import calc_SET_and_PMV
import calc_WBT_mm as calc_WBT
...
def WBGT(T, e, RH, p, version, WBT_vers='DavJon'):

    #Convert T to degCelsius
    T_C = convert_T(T, 'Celsius')
    
    if version=='simple':
        WBGT = 0.567 * T_C + 0.393 * e/100 + 3.94   #Simplified wet bulb globe temperature (Australian Bureau of Meteorology, 2010)
    elif version=='indoor':
        
        #Wet bulb temperature 
        if WBT_vers=='Stull':
            T_w = WBT_Stull(T, RH)
        elif WBT_vers=='DavJon':
            T_w = WBT_DavJon(T, RH, p)

        WBGT = 0.7 * T_w + 0.3 * T_C     #Wet-bulb globe temperature indoors (Knutson 2016, Epstein 2006)
    
    return WBGT


#Wet-bulb globe temperature (version if WBT already exists)
def WBGT2(T, T_w):
...
#Apparent temperature (Steadman 1984, Morabito 2016)
def AppTemp(T, e, v, Q, version):

    # wind speed v in 10 m height
    # T_mrt is mean radiant temperature (Steadman 1994)
    
    #Convert T to Celsius
    T_C = convert_T(T, 'Celsius')

    if version=='':
        AT = 0.92 * T_C + 0.22 * e/100 - 1.3              # Apparent temperature (Steadman 1984)
    elif version=='universal':
        AT = -2.7 + 1.04 * T_C + 0.2 * e/100 - 0.65 * v   # Universal apparent temperature (Li 2017, Steadman 1984)
    elif version=='indoor':
        AT = 0.89 * T_C + 0.382 * e/100 - 2.56            # Apparent temperature indoor (Morabito 2016)
    elif version=='shade':
        AT = T_C + 0.33 * e/100 - 0.7 * v - 4             # Apparent temperature shade (Morabito 2016, Steadman 1994)
    elif version=='sun':
        AT = T_C + 0.348 * e/100 - 0.7 * v + 0.7 * T_mrt / (v + 10) - 4.25      # Apparent temperature sun (Morabito 2016)

    return AT


# Wet bulb temperature (Davies-Jones 2008, Bolton 1980, Buzan 2015). This function is adapted from the Matlab function of Buzan et al. (https://github.com/jrbuzan/WetBulb.m/blob/master/WetBulb.m)
def WBT_Buzan(T, RH, p):

    #Select whether to use specific humidity (0) or relative humidity (1) as input
    HumidityMode = 1
     
    #Calculate WBT
    WBT = calc_WBT.WetBulb(T, p, RH, HumidityMode)
...
def UTCI(T, e):
    ...
    va = 0.5
    D_Tmrt = 0 # Assume that mean radiant temperature = temperature
